def puzzle_solver(pieces, width, height):
    ...
    for p in pieces:
        ...
        if p.tl is None and p.tr is None and p.bl is None and p.br is None:
            return []
    # unmatched = pieces_list.copy()
    # for i, p in enumerate(unmatched):
    #     if p.left is None or p.right is None or p.up is None or p.down is None:
    #         for r in unmatched:
    #             match(p, r)
    #     else:
    #         del unmatched[i]
    for p in pieces_list:
        if p.left is None or p.right is None or p.up is None or p.down is None:
            for r in pieces_list:
                match(p, r)
    # for p in pieces_list:
    #     print(f"id={p.id}",
    #         f"l={p.left.id}" if p.left is not None else None,
    #         f"u={p.up.id}" if p.up is not None else None,
    #         f"r={p.right.id}" if p.right is not None else None,
    #         f"d={p.down.id}" if p.down is not None else None
    #     )
    for p in pieces_list:
        if p.tl is None and p.tr is None and p.bl is None:
            ...
            break
    q[0][0] = p
    # print(height, list(range(1, height)))
...
class Piece:
    def __init__(self, data):
        ...
        if self.tl is None and self.tr is None:
            self.top = 0
        if self.tl is None and self.bl is None:
            self.left = 0
        if self.tr is None and self.br is None:
            self.right = 0
        if self.bl is None and self.br is None:
            self.bottom = 0

    def is_tl_corner(self):
        return self.tl is None and self.tr is None and self.bl is None and self.br is None


    def __repr__(self):
        return f"Piece(({self.tl, self.tr}), ({self.bl, self.br}), {self.id})"

def match(p1, p2):
    # if p1.left != 0 and not (p1.tl is None and p1.bl is None) and p1.tl == p2.tr and p1.bl == p2.br:
    #     p1.left = p2
    #     p2.right = p1
    #     return True
    if p1.right != 0 and not (p1.tr is None and p1.br is None) and p1.tr == p2.tl and p1.br == p2.bl:
        p1.right = p2
        p2.left = p1
        return True
    # if p1.up != 0 and not (p1.tl is None and p1.tr is None) and p1.tl == p2.bl and p1.tr == p2.br:
    #     p1.up = p2
    #     p2.down = p1
    #     return True
    if p1.down != 0 and not (p1.bl is None and p1.br is None) and p1.bl == p2.tl and p1.br == p2.tr:
        p1.down = p2
        p2.up = p1
        return True
    return False
